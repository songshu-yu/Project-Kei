...
- 2026-08-31 官方下载源可视化增量：模块中心新增本地内嵌 GitHub/Gitee 图标、双状态卡
  与“测试连通性”按钮；点击前零 POST，点击后唯一调用 PK-010 固定空请求，显示本次
  `可用|不可用`、响应时间和脱敏原因。测试期间冻结来源/刷新/批量交互，完成后恢复；
  页面加载、切换来源和展开仍零网络，窄屏单列且不依赖外部图标资源。
