"""Build a reviewable calendar directory or deterministic local ZIP package."""
...
OFFICIAL_ASSET_NAME = "calendar-1.0.0.zip"
...
def build_calendar_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-calendar-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "calendar", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the local calendar installable package")
    ...
    result = build_calendar_package(args.output, args.version)
    print(result)
    if result.is_file():
