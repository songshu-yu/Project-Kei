"""Build a reviewable focus directory or deterministic local ZIP package."""
...
OFFICIAL_RELEASE_VERSION = "1.1.1"
OFFICIAL_RELEASE_TAG = "modules-2026.08.02"
OFFICIAL_ASSET_NAME = "focus-1.1.1.zip"
...
def _write_zip(source: Path, destination: Path) -> None:
    ...
    # Stored entries avoid zlib-version-dependent output. Together with normalized
    # LF source bytes and explicit metadata, the archive is byte-for-byte stable
    # across the supported Python versions and operating systems.
...
def build_focus_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-focus-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "focus", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the local focus installable package")
    ...
    result = build_focus_package(args.output, args.version)
    print(result)
    if result.is_file():
