"""Build a reviewable intel_sources directory or deterministic ZIP package."""
...
OFFICIAL_RELEASE_VERSION = "1.1.6"
OFFICIAL_RELEASE_TAG = "modules-2026.08.08"
OFFICIAL_ASSET_NAME = "intel-sources-1.1.6.zip"
...
def build_intel_sources_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-intel-sources-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "intel_sources", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local intelligence-source registry installable package"
    )
    ...
    result = build_intel_sources_package(args.output, args.version)
    print(result)
    if result.is_file():
