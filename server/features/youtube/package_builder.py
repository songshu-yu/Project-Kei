"""Build a reviewable directory or deterministic YouTube module ZIP."""
...
OFFICIAL_ASSET_NAME = "youtube-1.0.1.zip"
...
def build_youtube_package(
    ...
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-youtube-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "youtube", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local YouTube Collector installable package"
    )
    ...
    result = build_youtube_package(args.output, args.version)
    print(result)
    if result.is_file():
