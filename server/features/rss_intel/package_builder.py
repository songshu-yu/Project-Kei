"""Build a reviewable directory or deterministic RSS intelligence ZIP."""
...
OFFICIAL_ASSET_NAME = "rss_intel-1.0.0.zip"
...
def build_rss_intel_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-rss-intel-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "rss_intel", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local RSS intelligence installable package"
    )
    ...
    result = build_rss_intel_package(args.output, args.version)
    print(result)
    if result.is_file():
