"""Build a reviewable fitness directory or deterministic local ZIP package."""
...
OFFICIAL_RELEASE_TAG = "modules-2026.08.08"
OFFICIAL_ASSET_NAME = "fitness-1.0.3.zip"
...
PACKAGE_INIT = '"""Project Kei installable fitness backend."""\n\nfrom .module import register, unregister\n\n__all__ = ["register", "unregister"]\n'
...
def _materialize_directory(destination: Path, version: str) -> Path:
    ...
    _write_utf8_lf(destination / "backend" / "__init__.py", PACKAGE_INIT)
...
def build_fitness_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-fitness-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "fitness", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the local fitness installable package")
    ...
    result = build_fitness_package(args.output, args.version)
    print(result)
    if result.is_file():
