"""Build a reviewable affection_memory directory or deterministic ZIP."""
...
OFFICIAL_RELEASE_VERSION = "1.0.4"
OFFICIAL_RELEASE_TAG = "modules-2026.08.02"
OFFICIAL_ASSET_NAME = "affection_memory-1.0.4.zip"
...
def build_affection_memory_package(
    destination: Path,
    ...
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(
            prefix="kei-affection-memory-package-"
        ) as temp_dir:
            source = _materialize_directory(
                Path(temp_dir) / "affection_memory", version
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local affection_memory installable package"
    )
    parser.add_argument("output", type=Path)
    parser.add_argument("--version", default=OFFICIAL_RELEASE_VERSION)
    args = parser.parse_args()
    result = build_affection_memory_package(args.output, args.version)
    print(result)
    if result.is_file():
