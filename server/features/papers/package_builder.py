"""Build a reviewable papers directory or deterministic local ZIP package."""
...
OFFICIAL_ASSET_NAME = "papers-1.0.0.zip"
...
def build_papers_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-papers-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "papers", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local papers installable package"
    )
    parser.add_argument("output", type=Path)
    parser.add_argument("--version", default=OFFICIAL_RELEASE_VERSION)
    args = parser.parse_args()
    result = build_papers_package(args.output, args.version)
    print(result)
    if result.is_file():
