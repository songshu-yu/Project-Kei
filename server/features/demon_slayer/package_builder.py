"""Build a reviewable demon-slayer directory or deterministic local ZIP."""
...
OFFICIAL_ASSET_NAME = "demon_slayer-1.0.1.zip"
...
def build_demon_slayer_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-demon-slayer-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "demon_slayer", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the local demon-slayer installable package")
    ...
    result = build_demon_slayer_package(args.output, args.version)
    print(result)
    if result.is_file():
