"""Build a reviewable directory or deterministic GitHub intelligence ZIP."""
...
OFFICIAL_ASSET_NAME = "github-intel-1.0.0.zip"
FIXED_ZIP_DATETIME = (2026, 1, 1, 0, 0, 0)
BACKEND_FILES = ("collector.py", "provider.py")
...
def build_github_intel_package(
    destination: Path,
    version: str = OFFICIAL_RELEASE_VERSION,
) -> Path:
    destination = Path(destination)
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="kei-github-intel-package-") as temp_dir:
            source = _materialize_directory(Path(temp_dir) / "github_intel", version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local GitHub intelligence installable package"
    )
    ...
    result = build_github_intel_package(args.output, args.version)
    print(result)
    if result.is_file():
