...
def parse_catalog_entry(payload: Any) -> CatalogEntry:
    ...
    download_url = _https_url(
        payload.get("download_url"), "download_url", hosts=set(hosts)
    )
    ...
    max_file_bytes = _positive_int(
        payload, "max_file_bytes", maximum=20 * 1024**3
    )
...
class VoicePackCatalog:
    ...
    def load(cls, root: Path) -> "VoicePackCatalog":
        ...
        for path in sorted(root.glob("*.json")):
            ...
            entry = parse_catalog_entry(payload)
    ...
    def get(self, key: str) -> CatalogEntry:
        try:
            ...
            raise CatalogError(
                "Voice Pack release is not present in the trusted catalog"
            ) from exc
