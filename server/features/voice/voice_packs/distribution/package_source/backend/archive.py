"""PK-213 outer release ZIP audit and safe extraction."""
...
def _safe_relative(value: str) -> PurePosixPath:
    if not value or "\\" in value:
        raise DistributionError(
            "Voice Pack ZIP path is invalid",
...
def _member_path(name: str, root: str) -> PurePosixPath:
    ...
    if not path.parts or path.parts[0] != root:
        raise DistributionError(
            "Voice Pack ZIP has an unexpected root",
            code="voice_pack_archive_unsafe",
        )
    return path
    ...
    with package:
        ...
        for info in infos:
            ...
            if stat.S_ISLNK(mode) or (
                mode and not (stat.S_ISREG(mode) or stat.S_ISDIR(mode))
            ):
                raise DistributionError(
        ...
        required = {
            f"{entry.archive_root}/{name}".casefold() for name in _FIXED_FILES
        }
