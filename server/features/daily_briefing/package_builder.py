"""Build a reviewable deterministic daily-briefing module package."""
...
BACKEND_FILES = (
    "__init__.py",
    "collector_contracts.py",
    "collector_gateway.py",
...
def build_daily_briefing_package(
    destination: Path,
    ...
    if destination.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(
            prefix="kei-daily-briefing-package-"
        ) as temp_dir:
            source = _materialize_directory(
                Path(temp_dir) / "daily_briefing",
                version,
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the local daily-briefing installable package"
    )
    parser.add_argument("output", type=Path)
    parser.add_argument("--version", default=OFFICIAL_RELEASE_VERSION)
    args = parser.parse_args()
    result = build_daily_briefing_package(args.output, args.version)
    print(result)
    if result.is_file():
